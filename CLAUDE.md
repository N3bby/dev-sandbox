...
docker-sandbox/              # everything the `dev` command uses
  ...
  kits/                      #   sbx mixin kits (--kit), selected per image variant
    claude-statusline/
    claude-dashboard/
    codex-config/            #   Codex defaults, statusline, and dashboard plugin
...
3. Starts a fresh sandbox for the current directory with a unique name (`dev-<slug>-<pid>-<rand>`), so repeated runs in one directory yield independent sandboxes. Mixin kits from `docker-sandbox/kits/` are selected per image variant and layered on via `--kit`: `shell` and `claude` get the Claude Code statusline and dashboard kits, while `codex` gets one kit containing its configuration, native TUI statusline, and dashboard plugin. The default agent is a plain `shell`; running `dev claude` launches the Claude Code agent, which additionally pins the model to a default; `dev codex` launches the Codex agent with the pre-vetted plugin hooks trusted. On exit (or Ctrl-C) the sandbox is stopped and removed via a trap.
...
- `.../agents/codex/config` (`mkdir`) → `~/.codex`, which holds codex's `config.toml`, `auth.json`, and session history. This works only because the image installs codex with its runtime relocated to `~/.codex-runtime` (see `docker-container/Dockerfile`); by default codex puts its runtime binaries under `~/.codex/packages`, so mounting an empty host dir there would shadow the binary and dangle the `~/.local/bin/codex` symlink. Note it must be a whole-dir mount, not a pre-seeded `auth.json`: codex reads an empty `{}` auth as "logged in", so seeding the file would falsely report a logged-in state.
